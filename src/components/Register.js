...
class Register extends React.Component {
  ...
  validateInput = () => {
    ...
    if((this.state.username.length < 6) || (this.state.password.length < 6)){
      message.error("Username or Password length can not be less than 6 characters!");
      return false;
    }
    if((this.state.username.length >32) || (this.state.password.length > 32)){
      message.error("Username or Password length can not be greater than 20 characters!");
      return false;
    }
