...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    let response = {};
    let errored = false;
    console.log("ID Delete : ", addressId)
    this.setState({
      loading: true,
    });
    ...
    if (this.validateResponse(errored, response, "delete a address")) {
      if (response) {
  ...
  order = async () => {
    ...
    } else if(!this.state.addresses || this.state.selectedAddressIndex === null){
      message.error("Address not selected");
    } else 
    this.checkout();
  };
  ...
  async componentDidMount() {
    if (localStorage.getItem("username") && localStorage.getItem("token")) {
      ...
      let items = this.cartRef.current.getCart();
      if(items.length === 0){
